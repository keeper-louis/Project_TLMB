...
namespace KEEPER.K3.TLMB.KAZXSSJCX
{
    [Description("KA组销售数据查询")]
    public class KAGroupSaleDataReport: SysReportBaseService
    {
        #region 参数设置
        ...
        public override void BuilderReportSqlAndTempTable(IRptParams filter, string tableName)
        {
            ...
            if (columnFilter == 2 || columnFilter == 3)
            {
            ...
            create table {0} as 
            select /*二开/KA组*/rownum FNUM1,a.*, {2}
            from(
        ...
        private void CreateTempTalbe()
        {
            ...
            string sqlstr1 = string.Format(sqlstr0, tempTable01);
            DBUtils.Execute(this.Context, sqlstr1);

            string sqlstr2 = string.Format(sqlstr0, tempTable02);
        ...
        private void InsertData(IRptParams filter)
        {
            ...
            string instr0 = @"/*dialect*/
                    ...
                    select/*二开/KA组*/ FSALEDEPTID,FSALEORGID,fforgroupcust ,FCUSTID, FMATERIALID,
                    ...
                    from (
                        select e.FSSKAZ FSALEDEPTID,b.FSALEORGID,e.fforgroupcust,b.FCUSTOMERID FCUSTID,a.FMATERIALID,
                                ...
                                where 1=1 {1} {2} {3} {5} {6} {7} and b.fdocumentstatus='C'
                        union all
                        select e.FSSKAZ FSALEDEPTID,b.FSALEORGID,e.fforgroupcust,b.FRETCUSTID FCUSTID,a.FMATERIALID,
                                ...
                                join T_BD_MATERIAL h on a.FMATERIALID = h.FMATERIALID
                                where 1=1 {1} {2} {4} {5} {6} and b.fdocumentstatus='C'
                        union all
                        select e.FSSKAZ FSALEDEPTID,b.FSALEORGID,e.fforgroupcust,b.FRETCUSTID FCUSTID,a.FMATERIALID,
                                ...
                                join T_BD_CUSTOMER e on b.FRETCUSTID = e.FCUSTID
                                join T_BD_MATERIAL h on a.FMATERIALID = h.FMATERIALID
                                where 1=1 {1} {2} {4} {5} {6} and b.fdocumentstatus='C' and b.FRETURNTYPE0='2'
                    )";
            #endregion

            #region KA组今年、去年销售数据
            //获取过滤条件值FISFREE = 0
            string FDept = FDeptFilter(filter); string FMaterial = FMaterialIDFilter(filter); string FOutCust = FOutCustFilter(filter);
            string FRetCust = FRetCustFilter(filter); string FWL = FWLFilter(filter); string FSFZP = FSFZPFilter(filter);
            //今年
            string instr1 = string.Format(instr0, tempTable01, FDept, FMaterial, FOutCust, FRetCust, GetFilterSql(filter), FWL, FSFZP);
            DBUtils.Execute(this.Context, instr1);
            //去年
            string instr2 = string.Format(instr0, tempTable02, FDept, FMaterial, FOutCust, FRetCust, GetFilterSqlLast(filter), FWL, FSFZP);
            ...
            select /*二开/KA组*/FNAME,a.FID,nvl(FSTOCKNUM,0) FSTOCKNUM,nvl(FSTOCKQUOTA,0) FSTOCKQUOTA,
            ...
            insert into {0}(FNAME,FID, FSTOCKNUM, FSALENUM, FRETURNNUM, FSTOCKQUOTA, FSALEQUOTA, FRETURNQUOTA, FSALEPER, FRETURNPER,
                          FSTOCKNUM_LAST, FSALENUM_LAST, FRETURNNUM_LAST, FSTOCKQUOTA_LAST, FSALEQUOTA_LAST, FRETURNQUOTA_LAST, 
                          FSALEPER_LAST, FRETURNPER_LAST,FSALEQUOTAPER)
            select/*二开/KA组*/ FNAME,FID,FSTOCKNUM, FSALENUM, FRETURNNUM, FSTOCKQUOTA, FSALEQUOTA, FRETURNQUOTA, FSALEPER, FRETURNPER,
                          FSTOCKNUM_LAST, FSALENUM_LAST, FRETURNNUM_LAST, FSTOCKQUOTA_LAST, FSALEQUOTA_LAST, FRETURNQUOTA_LAST, 
        ...
        public override ReportTitles GetReportTitles(IRptParams filter)
        {
            ...
            string s1 = dep["Name"].ToString();
            //string s2 = Convert.ToString(filter.FilterParameter.CustomFilter["FFORGROUPCUST_ID"]);
            //string s3 = string.Format(@"select FDATAVALUE from T_BAS_ASSISTANTDATAENTRY_L WHERE fentryid='{0}'", s2);
            ...
            switch (columnFilter)
            {
                case 1:
                    s = "KA组-产品-客户";
                    DynamicObject mat = (DynamicObject)filter.FilterParameter.CustomFilter["FMATERIALID"];
                    tableTitle = string.Format(@"{0}{1}销售数据查询", s1, mat["Name"].ToString());
                    break;
                case 2:
                    s = "KA组-客户-产品";
                    DynamicObject cus = (DynamicObject)filter.FilterParameter.CustomFilter["FCUSTOMER"];
                    tableTitle = string.Format(@"{0}{1}销售数据查询", s1, cus["Name"].ToString());
                    break;
                case 3:
                    s = "KA组-产品";
                    tableTitle = string.Format(@"{0}销售数据查询", s1);
                    break;
                case 4:
                    s = "KA组-客户";
                    tableTitle = string.Format(@"{0}销售数据查询", s1);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        private string FOutCustFilter(IRptParams filter)
        {

            long outCustID = Convert.ToInt64(filter.FilterParameter.CustomFilter["FCUSTOMER_Id"]);
            if (outCustID == 0)
            {
                return "";
            }
            return string.Format("and b.FCUSTOMERID={0}", outCustID);
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        private string FRetCustFilter(IRptParams filter)
        {

            long retCustID = Convert.ToInt64(filter.FilterParameter.CustomFilter["FCUSTOMER_Id"]);
            if (retCustID == 0)
            {
                return "";
            }
            return string.Format("and b.FRETCUSTID={0}", retCustID);
        ...
        private String ColumnFilter(IRptParams filter)
        {
            switch (columnFilter)
            {
                case 1: return "FCUSTID";
                case 2: return "FMATERIALID";
                case 3: return "FMATERIALID";
                case 4: return "FCUSTID";
