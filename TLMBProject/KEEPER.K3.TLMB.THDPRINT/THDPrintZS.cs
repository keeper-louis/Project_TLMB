...
from  t_bd_material spp 
 left join 
(
 select tss.fpickdate,tss.fsalerid,fmaterialid, sum(FQTY) FQTY,sum(FPACKERQTY) FPACKERQTY from T_STK_STKTRANSFERIn tss
...
where he.fline=" + dr["fid"].ToString() + @" and tss.fid=tse.fid   and  fsaledeptid={2} and tss.fpickdate= to_date('{3}','YYYY/MM/DD') AND tss.fsalerid<>0  
group by tss.fpickdate,tss.fsalerid,fmaterialid) sss on spp.fmaterialid=sss.fmaterialid    
